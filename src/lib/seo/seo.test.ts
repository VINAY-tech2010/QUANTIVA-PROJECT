...
describe("buildMetadata", () => {
  it("emits a canonical URL and default OG image", () => {
    ...
    expect(meta.alternates?.canonical).toBe("https://quantiva.app/test");
    expect(meta.openGraph?.images).toEqual([
      { url: "https://quantiva.app/opengraph-image", width: 1200, height: 630, alt: "QUANTIVA — Test" },
    ]);
  });
  ...
  it("uses a page-specific OG image when provided", () => {
    ...
    expect(meta.openGraph?.images).toEqual([
      { url: "https://quantiva.app/calculator/loan/opengraph-image", width: 1200, height: 630, alt: "QUANTIVA — Loan Calculator" },
    ]);
  });
